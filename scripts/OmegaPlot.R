...
for(i in 1:nrow(insList)){      # for each insertion
  ...
  if(type==0){
    ...
    for(y in 1:nrow(ctrlList)){          # Reading all control runs and keeping the max value
      ...
      ctrlfile=paste(CTRLResPATH,"OmegaPlus_Report.chr",chr,".",pos,".",type,".at.chr",controlCHR,".",controlNUM,".grid.run",sep="")
  ...
  # Reading the OmegaPlus run in the insertion area and cleaning it from the last and the first 4 lines, that are useless
  ins=read.table(file=paste(ResPATH,"OmegaPlus_Report.chr",chr,".",pos,".",type,".run",sep=''),skip=4)
...
## Ploting Time
x0=1;x1=1;
pdf(paste(paste("omegaplus.plot.pdf", sep='')), height = 5*(nrow(insList)/2), width = 10)
...
for( j in 1:nrow(insList)){       # for each insertion
  ...
  if(type==0){
    plot(insData0[[x0]][,1], insData0[[x0]][,2], pch=19, cex=0.7, ylim=c(0,max(insData0[[x0]][,2]*1.2, thresholds0[x0]*1.2)), xlab="Position", ylab="omega-statistic", main=paste("chr: ", insChr, " Position: ", insPos, " Type: 00" ))
    abline(h=thresholds0[x0], col="red")    # drawing a red line where the threshold is defined
    x0=x0+1
  }else{
    plot(insData1[[x1]][,1], insData1[[x1]][,2], pch=19, cex=0.7, ylim=c(0,max(insData1[[x1]][,2]*1.2, thresholds1[x1]*1.2)), xlab="Position", ylab="omega-statistic", main=paste("chr: ", insChr, " Position: ", insPos, " Type: 11" ))
